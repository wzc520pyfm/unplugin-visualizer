...
function getBytesPerFileUsingSourceMap(bundleId: string, code: string, map: SourceMapConsumer, dir: string) {
  ...
  for (let i = 0; i < codeChars.length; i++, column++) {
    ...
    if (source != null) {
      ...
      modules[id].renderedLength += buffer.Buffer.byteLength(char)
...
export function getSourcemapModules(id: string, outputChunk: OutputChunk, dir: string): Promise<Record<string, SourceMapModuleRenderInfo>> {
