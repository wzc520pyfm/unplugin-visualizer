...
function addToPath(moduleId: string, tree: ModuleTree, modulePath: string[], node: MappedNode): void {
...
function mergeSingleChildTrees(tree: ModuleTree): ModuleTree | ModuleTreeLeaf {
...
export function buildTree(bundleId: string, modules: Array<ModuleLengths & { id: string }>, mapper: ModuleMapper): ModuleTree {
...
export function mergeTrees(trees: Array<ModuleTree | ModuleTreeLeaf>): ModuleTree {
  const newTree = {
    name: 'root',
    children: trees,
    isRoot: true,
...
export function addLinks(startModuleId: string, getModuleInfo: GetModuleInfo, mapper: ModuleMapper): void {
