...
import type { NetworkNode, NodeInfo } from './index'
import { StaticContext } from './index'

export const Main: FunctionalComponent = () => {
  const { nodes, data, width, height, nodeGroups, groupLayers, groups }
  ...
  const getColor = (node: NodeInfo) => {
    if (selectedNode != null) {
      if (
        ...
        || data.nodeMetas[selectedNode].importedBy.some(
          ({ uid }) => node.uid === uid,
        )
      )
  ...
  const processedNodes = useMemo(() => {
    ...
    for (const node of nodes) {
      ...
      const layerIndex = groupLayers.findIndex(layer =>
        layer.includes(nodeGroup),
      )
  ...
  return (
    <>
      ...
      <Chart
        ...
        getColor={getColor}
        onNodeSelect={uid =>
          setSelectedNode(uid === selectedNode ? undefined : uid)}
      />
    </>
  )
}
